...
import { replyMarkdown } from './send';
...
export async function createTelegramBot(params: {
  ...
  bot.on('message:photo', async (ctx) => {
    try {
      ...
      if (!fallback) {
        await replyMarkdown(ctx, 'Не смог обработать картинку.', ctx.message.message_id);
        return;
      }
  ...
  bot.on('message:document', async (ctx) => {
    ...
    if (!runtime) return;
    await ctx.api.sendChatAction(ctx.message.chat.id, 'typing').catch((error) =>
      logger.debug('Could not send typing action', error),
    );

    const document = ctx.message.document;
    if (!isIdentityDocument(document.file_name, document.mime_type)) {
      await replyMarkdown(ctx, 'Identity можно задать только `.txt` или `.md` файлом.', ctx.message.message_id);
      return;
    }
    if (document.file_size && document.file_size > params.config.agentIdentityMaxChars * 4) {
      await replyMarkdown(ctx, `Файл слишком большой. Лимит: ${params.config.agentIdentityMaxChars} символов.`, ctx.message.message_id);
      return;
    }
    ...
    try {
      ...
      await replyMarkdown(ctx, `Identity сохранена для этого чата (${saved.length} символов).`, ctx.message.message_id);
    } catch (error) {
      logger.warn('Could not save identity from Telegram document', error);
      await replyMarkdown(ctx, 'Не смог прочитать identity-файл. Проверь, что это UTF-8 `.txt` или `.md`.', ctx.message.message_id);
    }
  });
...
async function handleIncomingChatMessage(
  ...
  if (!runtime) return;
  if (shouldShowTyping(message, botUsername)) {
    await ctx.api.sendChatAction(message.chatId, 'typing').catch((error) =>
      logger.debug('Could not send typing action', error),
    );
  }
  ...
  const sent = await replyMarkdown(ctx, reply, ctx.message.message_id);
